// Currency selector + Firebase (Realtime Database via REST) integration
// 1) Update conversion RATES as needed
// 2) Fill firebaseBaseUrl with your Firebase Realtime Database URL (e.g. https://your-project-id.firebaseio.com)

(function (){
    const RATES = {
        USD: 1,
        // approximate — update to current rates
        THB: 37,
        MMK: 4000
    };
    ...
    // Prefer explicit per-currency values if provided on the element.
    // You can set attributes like `data-price-thb="฿1200"` or `data-price-mmk="45,000 MMK"`.
    // If the currency-specific attribute is missing, we fall back to converting from `data-price-usd`.
    function formatPriceForElement(el, currency){
        // Use explicit per-currency attribute when present (no calculation).
        // e.g. data-price-thb="฿1,100" or data-price-mmk="110,000 MMK"
        ...
        // If USD is selected and data-price-usd exists, display it (format if numeric).
        if(currency === 'USD'){
            const usdAttr = el.getAttribute('data-price-usd');
            if(usdAttr !== null && usdAttr !== ''){
                // If numeric, format as $xx.xx; otherwise, use as provided.
        ...
        // No explicit price for this currency — leave element text as-is.
    ...
    // Replace the url below with your database root (no trailing slash), e.g. https://your-project-id.firebaseio.com
